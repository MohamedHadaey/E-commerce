        ...
        <div className="footer-content mx-auto">
          <div className="md:flex md:justify-between md:items-center mb-8">
            <div className="mb-6 md:mb-0">
              <h3 className=" mb-2">{t('footer.getApp')}</h3>
              <p className="text-sm text-gray-500 mb-3">{t('footer.appDescription')}</p>
              <form className="flex gap-2 w-full max-w-md">
                <input type="email" name="email" id="visitor-email" placeholder={t('footer.emailPlaceholder')} className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:border-green-600" />
                <button type="button" className="main-btn bg-green-600 text-white px-4 py-2 rounded text-sm font-medium hover:bg-green-700 transition">{t('footer.shareAppLink')}</button>
              </form>
            </div>
          ...
          <div className="flex flex-col md:flex-row md:justify-between md:items-center border-t border-gray-200 pt-4">
            <div className='d-flex flex-row   gap-6 md:gap-12 mb-4 md:mb-0'>
              <p className="font-medium mb-2 ">{t('footer.paymentPartners')}</p>
            ...
            <div className="flex items-center gap-4">
              <span className="text-sm text-gray-500">{t('footer.getDeliveries')}</span>
